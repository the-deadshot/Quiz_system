...
public class MyListener implements ServletContextListener{
	...
	try{
		...
		stmt.executeUpdate("CREATE TRIGGER  BI_QUIZINFO before insert on QUIZINFO for each row begin select QUIZ.nextval into :NEW.QUIZNAME from dual;end");
		stmt.executeUpdate("CREATE TRIGGER  BI_QUIZQUES before insert on QUIZQUES for each row begin select QUIZ.nextval into :NEW.QID from dual;end");
		}	
